...
struct ButtonLogic: View {
    ...
    // 狀態變數，pivate: 只能在 ContentView 內部使用
...
struct BackgroundPicture: View {
    ...
    var body: some View {
        colorScheme == .dark
        ? Image("暗寶可夢桌布") // Dark mode background
            .resizable()
            .scaledToFill()
            .frame(width: UIScreen.main.bounds.width * 0.1)
            .edgesIgnoringSafeArea(.all)
        : Image("寶可夢桌布") // Light mode background
            .resizable()
            .scaledToFill()
            .frame(width: UIScreen.main.bounds.width * 0.1)
            .edgesIgnoringSafeArea(.all)
